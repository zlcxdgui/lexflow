...
export async function PATCH(req: NextRequest, context: RouteContext) {
  try {
    ...
    const upstream = await fetch(`${apiUrl}/deadlines/${id}`, {
      method: 'PATCH',
      headers: {
    ...
    if (!upstream.ok) {
      return new NextResponse(String(extractMessage(text, 'Falha ao atualizar prazo')), { status: upstream.status, headers: { 'content-type': 'text/plain; charset=utf-8' } });
    }
    ...
    const message = err instanceof Error ? err.message : 'Erro no proxy de prazo';
...
export async function DELETE(_req: NextRequest, context: RouteContext) {
  try {
    ...
    const contentType = upstream.headers.get('content-type') || '';
    if (!upstream.ok) {
      return new NextResponse(String(extractMessage(text, 'Falha ao excluir prazo')), { status: upstream.status, headers: { 'content-type': 'text/plain; charset=utf-8' } });
    }
