...
async function getParams(
  context: { params: { tenantId: string; memberId: string } | Promise<{ tenantId: string; memberId: string }> }
) {
...
export async function PATCH(
  ...
  if (!tenantId || !memberId) return new NextResponse(String('Parâmetros inválidos'), { status: 400, headers: { 'content-type': 'text/plain; charset=utf-8' } });

  const body = await req.text();
  const upstream = await fetch(
    `${apiUrl}/tenants/${tenantId}/members/${memberId}`,
    {
      method: 'PATCH',
...
export async function GET(
  ...
  const upstream = await fetch(`${apiUrl}/tenants/${tenantId}/members/${memberId}`, {
    method: 'GET',
