...
export async function GET() {
  ...
  if (!resp.ok) {
    return new NextResponse(String(extractMessage(text, 'Não foi possível carregar perfil')), { status: resp.status, headers: { 'content-type': 'text/plain; charset=utf-8' } });
  }
...
export async function POST(req: NextRequest) {
  ...
  const contentType = resp.headers.get('content-type') || '';
  if (!resp.ok) {
    return new NextResponse(String(extractMessage(text, 'Não foi possível alterar senha')), { status: resp.status, headers: { 'content-type': 'text/plain; charset=utf-8' } });
  }
