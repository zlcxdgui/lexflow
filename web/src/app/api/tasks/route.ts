...
export async function GET(req: NextRequest) {
  try {
    ...
    const upstream = await fetch(`${apiUrl}/tasks${req.nextUrl.search}`, {
      method: 'GET',
    ...
    if (!upstream.ok) {
      return NextResponse.json(
        { message: extractMessage(text, 'Falha ao carregar agenda de tarefas') },
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!upstream.ok) {
      return NextResponse.json(
        { message: extractMessage(text, 'Falha ao criar tarefa') },
        { status: upstream.status },
      );
    }
