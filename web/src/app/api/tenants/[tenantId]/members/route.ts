...
async function getTenantId(context: { params: { tenantId: string } | Promise<{ tenantId: string }> }) {
...
export async function GET(
  _req: NextRequest,
  context: { params: { tenantId: string } | Promise<{ tenantId: string }> }
) {
  ...
  if (!tenantId) return new NextResponse(String('Tenant inválido'), { status: 400, headers: { 'content-type': 'text/plain; charset=utf-8' } });

  const upstream = await fetch(`${apiUrl}/tenants/${tenantId}/members`, {
    method: 'GET',
