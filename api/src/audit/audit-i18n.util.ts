export function formatAuditActionLabel(action: string): string {
  const map: Record<string, string> = {
    ...
    BILLING_PLAN_CHANGE_REQUEST_REVIEWED:
      'Solicitação de mudança de plano revisada',
    BILLING_CANCEL_AT_PERIOD_END_UPDATED:
      'Cancelamento no fim do período alterado',
    ...
    FINANCE_RECURRENCE_TEMPLATE_CREATED:
      'Template recorrente financeiro criado',
    FINANCE_RECURRENCE_TEMPLATE_UPDATED:
      'Template recorrente financeiro atualizado',
    FINANCE_RECURRENCE_GENERATED:
      'Lançamento financeiro gerado por recorrência',
    ...
    FINANCE_CATALOG_COST_CENTER_UPDATED:
      'Centro de custo financeiro atualizado',
...
export function formatAuditMetaEntries(
  ...
  return Object.entries(meta || {})
    ...
    .map(
      ([k, v]) =>
        `${formatAuditMetaKeyLabel(k)}: ${formatAuditMetaValueText(k, v)}`,
    )
    .join(' · ');
}
