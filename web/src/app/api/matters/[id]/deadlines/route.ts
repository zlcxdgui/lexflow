...
export async function GET(req: NextRequest, context: RouteContext) {
  try {
    ...
    const upstream = await fetch(`${apiUrl}/matters/${id}/deadlines`, {
      method: 'GET',
    ...
    if (!upstream.ok) {
      return new NextResponse(String(extractMessage(text, 'Falha ao carregar prazos')), { status: upstream.status, headers: { 'content-type': 'text/plain; charset=utf-8' } });
    }
    ...
    const message = err instanceof Error ? err.message : 'Erro no proxy de prazos';
...
export async function POST(req: NextRequest, context: RouteContext) {
  try {
    ...
    const contentType = upstream.headers.get('content-type') || '';
    if (!upstream.ok) {
      return new NextResponse(String(extractMessage(text, 'Falha ao criar prazo')), { status: upstream.status, headers: { 'content-type': 'text/plain; charset=utf-8' } });
    }
